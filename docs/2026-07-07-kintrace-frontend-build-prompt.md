...
> **Transcription shape (ingestion upgrade LANDED).** The ingestion-upgrade work (`docs/2026-07-08-kintrace-ocr-htr-ingestion-prompt.md`) replaced the old single `transcription` field with a **diplomatic** + **normalized** pair and structured per-item confidence data from a two-pass HTR pipeline. The confidence object is pinned: `ai_confidence = { overall: 'high' | 'medium' | 'low', summary: string, flaggedSpans: [{ text: string, reason: string }] } | null` — `GET /api/items/:id` returns it **already parsed as an object** (unlike `ai_names`, which stays a JSON string), and it is **not PATCHable** (it describes the AI draft; human edits supersede it). `flaggedSpans[].text` quotes the uncertain span as it appears in the diplomatic text (usually containing an uncertainty marker) so the review UI can locate and highlight it; `reason` says why it's uncertain.
...
- `GET /api/items/:id` → full item incl. `transcription_diplomatic` (faithful text preserving line breaks + uncertainty markers `[illegible]`/`[?]`/`[possibly Name]`), `transcription_normalized` (modernized, search-friendly), the structured **`ai_confidence`** object (shape pinned in the note above; `null` until transcribed), `ai_names` (JSON string), `ai_error`, and `people: [{ id, name, role }]`. 404 if missing. (Either transcription may be null when the media has no text.)
